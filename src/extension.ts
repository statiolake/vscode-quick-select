...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      singleSelect.bind(_, { char: "`", multiline: true })
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      matchingSelect.bind(_, { start_char: "(", end_char: ")", outer: true })
    )
  );
  context.subscriptions.push(
    vscode.commands.registerCommand(
...
function findSingleSelect(
  ...
  if (matches.length > 1 && matches.length % 2 === 0) {
    ...
    let end_pos: vscode.Position = findNext(
      doc,
      start_pos.line,
      char,
...
function switchQuotes() {
  ...
  sel.map((s: vscode.Selection) => {
    ...
    let start_pos = new vscode.Position(
      s.start.line,
